...
export function TriggeredControlsPanel({ result }: { result: PlaygroundCheckResult | null }) {
  ...
  return (
    <section className="overflow-hidden rounded-xl border bg-card shadow-xs">
      <PanelHeader icon={<SearchCheck />} title={t("playground.triggeredControls")} meta={result ? t("playground.controlCount", { count: result.controls.length }) : undefined} />
      {result ? result.controls.length ? (
        <div className="divide-y">
          {result.controls.map((control) => {
            const matched = control.status === "matched";
            return <div key={control.id} className={cn("grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-3", matched ? "bg-red-50/50" : "text-muted-foreground")}><div className="min-w-0"><strong className={cn("block truncate text-sm font-medium", matched && "text-foreground")}>{control.name}</strong><span className={cn("mt-1 inline-flex items-center gap-1.5 text-[11px]", matched ? "text-red-700" : control.status === "error" ? "text-amber-700" : "text-muted-foreground")}>
              {matched ? <ShieldAlert className="size-3.5" /> : control.status === "error" ? <CircleAlert className="size-3.5" /> : <CheckCircle2 className="size-3.5 text-emerald-600" />}
              {t(`playground.controlStates.${control.status}`)}
            </span></div><span className="font-mono text-xs tabular-nums text-muted-foreground">{control.duration_ms} ms</span></div>;
          })}
        </div>
      ) : <PanelEmpty text={t("playground.noControlsEvaluated")} /> : <PanelEmpty text={t("playground.runToSeeControls")} />}
    </section>
  );
}
    ...
    <section className="overflow-hidden rounded-xl border bg-card shadow-xs">
      <details className="group">
        ...
        <div className="border-t bg-muted/[0.08] p-3">
          {result ? result.trace.length ? <div className="space-y-2">{result.trace.map((step) => <div key={step.id} className="rounded-lg border bg-card p-3"><div className="flex items-center gap-2"><span className="rounded border bg-muted/40 px-1.5 py-0.5 text-[10px] font-medium uppercase text-muted-foreground">{step.kind ?? t("playground.step")}</span><strong className="min-w-0 flex-1 truncate text-xs font-medium">{step.name}</strong><span className="font-mono text-[10px] text-muted-foreground">{step.duration_ms} ms</span></div><p className="mt-1.5 text-[11px] leading-5 text-muted-foreground">{step.detail}</p><p className="mt-1 text-[10px] font-medium uppercase tracking-wide text-muted-foreground">{step.verdict ?? step.status}</p></div>)}</div> : <PanelEmpty text={t("playground.noTraceSteps")} /> : <PanelEmpty text={t("playground.runToSeeTrace")} />}
        </div>
      </details>
    </section>
  );
