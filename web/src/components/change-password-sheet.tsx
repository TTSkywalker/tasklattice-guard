...
export function ChangePasswordSheet({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const canSubmit = Boolean(currentPassword) && newPassword.length >= 10 && confirmation.length >= 10 && !mutation.isPending;
    ...
    >
      <form id={FORM_ID} className="grid gap-5" onSubmit={submit}>
        ...
        <Field label={t("auth.newPassword")} hint={t("auth.passwordHint")}>
          <Input className="h-11" type="password" autoComplete="new-password" minLength={10} value={newPassword} onChange={(event) => setNewPassword(event.target.value)} required />
        </Field>
        <Field label={t("auth.confirmNewPassword")}>
          <Input className="h-11" type="password" autoComplete="new-password" minLength={10} value={confirmation} onChange={(event) => setConfirmation(event.target.value)} required />
        </Field>
