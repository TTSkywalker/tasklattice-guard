      ...
      {integrations.length ? (
        <>
          <section className="mt-5 surface">
            ...
            <div className="divide-y divide-border">{integrations.map((integration) => <button key={integration.id} type="button" onClick={() => setSelected(integration)} className="group relative grid min-h-20 w-full gap-4 p-5 text-left transition-colors hover:bg-muted/50 focus-visible:outline-2 focus-visible:outline-ring md:grid-cols-[minmax(0,1fr)_170px_170px_130px_28px] md:items-center"><div><span className="flex items-center gap-2.5"><ProtocolIcon protocol={integration.protocol} size="sm" /><strong className="text-sm font-medium">{integration.name}</strong></span><span className="mt-1.5 block text-xs text-muted-foreground">{t(`integrations.protocols.${integration.protocol}`)}</span></div><span className="text-xs capitalize">{integration.environment}</span><span className="font-mono text-xs">{t("integrations.requestErrorCount", { requests: integration.request_count, errors: integration.error_count })}</span><StateBadge state={integration.runtime_status} /><ChevronRight className="absolute right-4 top-5 size-4 text-muted-foreground md:static" /></button>)}</div>
          </section>
        </>
...
function IntegrationDetail({ integration, onOpenChange }: { integration: Integration | null; onOpenChange: (open: boolean) => void }) {
  ...
  return <EntitySheet open onOpenChange={onOpenChange} eyebrow={t("integrations.details")} title={integration.name} description={t("integrations.detailsDescription")} width="md" footer={<Button variant="outline" onClick={() => onOpenChange(false)}>{t("common.close")}</Button>}>
    <div className="space-y-5">
      <section className="overflow-hidden rounded-lg border bg-card"><div className="flex items-center justify-between border-b bg-muted/40 p-4"><div className="flex items-center gap-3"><ProtocolIcon protocol={integration.protocol} /><div><p className="text-sm font-medium">{integration.protocol.toUpperCase()}</p><p className="mt-0.5 text-xs capitalize text-muted-foreground">{integration.environment}</p></div></div><StateBadge state={integration.runtime_status} /></div><dl className="divide-y divide-border"><Detail label={t("integrations.id")} mono copyValue={integration.id}>{integration.id}</Detail><Detail label={t("integrations.protocol")}>{t(`integrations.protocols.${integration.protocol}`)}</Detail><Detail label={t("integrations.credential")} mono>{integration.credential_prefix || t("integrations.waitingCredential")}</Detail><Detail label={t("integrations.verification")}><StateBadge state={integration.verification_status} /></Detail></dl></section>
