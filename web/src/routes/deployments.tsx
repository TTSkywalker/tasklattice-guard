            ...
            <section className="mt-4 rounded-lg border border-dashed bg-muted/20 px-5 py-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-3">
                  ...
                  <div><p className="text-sm font-medium">{t("deployments.defaultName")}</p><p className="mt-1 text-xs text-muted-foreground">{t("deployments.defaultDescription")}</p></div>
                </div>
          ...
          return (
            <article key={deployment.id} className="grid gap-4 px-5 py-4 xl:grid-cols-[104px_minmax(160px,1fr)_minmax(220px,1.5fr)_minmax(150px,.8fr)_150px] xl:items-center">
              <div className="flex flex-col items-start gap-1">
                ...
                <div className="flex">
                  <Button type="button" variant="ghost" size="icon" className="size-11" aria-label={t("deployments.moveUp", { name: deployment.name })} disabled={reordering || catchAll || index === 0} onClick={() => move(index, -1)}><ArrowUp className="size-3.5" /></Button>
