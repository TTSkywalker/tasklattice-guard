    ...
    <section className="py-6 sm:py-8">
      ...
      {!runsQuery.isLoading && guardrails.length ? (
        <section className="mt-5 overflow-hidden rounded-xl border bg-card shadow-xs">
          ...
          {filtered.length ? <Table><TableHeader><TableRow className="hover:bg-transparent"><TableHead className="pl-4">{t("validation.validationRunColumn")}</TableHead><TableHead>{t("validation.guardrailColumn")}</TableHead><TableHead>{t("validation.targetColumn")}</TableHead><TableHead>{t("validation.casesColumn")}</TableHead><TableHead>{t("validation.statusColumn")}</TableHead><TableHead>{t("validation.passRateColumn")}</TableHead><TableHead>{t("validation.durationColumn")}</TableHead><TableHead>{t("validation.runAtColumn")}</TableHead><TableHead className="w-12"><span className="sr-only">{t("validation.openValidationRun")}</span></TableHead></TableRow></TableHeader><TableBody>{filtered.map((run) => <ValidationRow key={run.id} run={run} guardrailName={guardrailNames.get(run.guardrail_id)} locale={i18n.language} onOpen={() => setSelected(run)} />)}</TableBody></Table> : <EmptyState title={runs.length ? t("validation.noMatchingValidationRuns") : t("validation.noValidationRuns")} description={runs.length ? t("validation.noMatchingValidationRunsDescription") : t("validation.noValidationRunsDescription")} action={!runs.length ? <Button onClick={() => setCreateOpen(true)}><Plus />{t("validation.createValidationRun")}</Button> : undefined} />}
        </section>
      ) : null}
