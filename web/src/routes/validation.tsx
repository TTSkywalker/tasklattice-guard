...
export function ValidationPage() {
  ...
  return (
    <section className="py-6 sm:py-8">
      <PageHeader title={t("pages.validation.title")} description={t("pages.validation.description")} action={<Button className="min-h-11" disabled={!guardrails.length} onClick={() => setCreateOpen(true)}><Plus />{t("validation.createValidationRun")}</Button>} />
      ...
      {!runsQuery.isLoading && guardrails.length ? (
        <>
          <ValidationSummary runs={runs} />
          <section className="mt-5 overflow-hidden rounded-xl border bg-card shadow-xs">
            <div className="grid gap-3 border-b bg-muted/20 p-4 sm:grid-cols-[minmax(16rem,1fr)_14rem_auto] sm:items-center"><label className="relative"><span className="sr-only">{t("validation.searchValidationRuns")}</span><Search className="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="min-h-11 bg-card pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t("validation.searchValidationRuns")} /></label><Select value={status} onValueChange={setStatus}><SelectTrigger className="min-h-11 bg-card" aria-label={t("validation.filterStatus")}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">{t("validation.allStatuses")}</SelectItem><SelectItem value="passed">{t("states.passed")}</SelectItem><SelectItem value="failed">{t("states.failed")}</SelectItem><SelectItem value="incomplete">{t("states.incomplete")}</SelectItem></SelectContent></Select><p className="text-xs text-muted-foreground sm:text-right">{t("validation.validationRunCount", { count: filtered.length })}</p></div>
            {filtered.length ? <Table><TableHeader><TableRow className="hover:bg-transparent"><TableHead className="pl-4">{t("validation.validationRunColumn")}</TableHead><TableHead>{t("validation.guardrailColumn")}</TableHead><TableHead>{t("validation.targetColumn")}</TableHead><TableHead>{t("validation.casesColumn")}</TableHead><TableHead>{t("validation.statusColumn")}</TableHead><TableHead>{t("validation.passRateColumn")}</TableHead><TableHead>{t("validation.durationColumn")}</TableHead><TableHead>{t("validation.runAtColumn")}</TableHead><TableHead className="w-12"><span className="sr-only">{t("validation.openValidationRun")}</span></TableHead></TableRow></TableHeader><TableBody>{filtered.map((run) => <ValidationRow key={run.id} run={run} guardrailName={guardrailNames.get(run.guardrail_id) ?? run.guardrail_id} locale={i18n.language} onOpen={() => setSelected(run)} />)}</TableBody></Table> : <EmptyState title={runs.length ? t("validation.noMatchingValidationRuns") : t("validation.noValidationRuns")} description={runs.length ? t("validation.noMatchingValidationRunsDescription") : t("validation.noValidationRunsDescription")} action={!runs.length ? <Button onClick={() => setCreateOpen(true)}><Plus />{t("validation.createValidationRun")}</Button> : undefined} />}
          </section>
        </>
      ) : null}
      ...
      <ValidationDetailSheet run={selected} guardrail={selected ? guardrails.find((item) => item.id === selected.guardrail_id) : undefined} running={rerun.isPending} onRunAgain={(guardrailId) => rerun.mutate(guardrailId)} onClose={() => setSelected(null)} />
    </section>
  );
}

function ValidationSummary({ runs }: { runs: ValidationRun[] }) {
  const { t } = useTranslation();
  const passed = runs.filter((run) => run.status === "passed").length;
  const failed = runs.filter((run) => run.status === "failed").length;
  const latest = runs[0];
  return <dl className="mt-5 grid overflow-hidden rounded-xl border bg-card shadow-xs sm:grid-cols-3"><SummaryFact label={t("validation.totalValidationRuns")} value={String(runs.length)} /><SummaryFact label={t("validation.passedFailed")} value={`${passed} / ${failed}`} /><SummaryFact label={t("validation.latestPassRate")} value={latest ? `${latest.metrics.compliance_rate}%` : "—"} /></dl>;
}
...
function ValidationDetailSheet({ run, guardrail, running, onRunAgain, onClose }: { run: ValidationRun | null; guardrail?: Guardrail; running: boolean; onRunAgain: (guardrailId: string) => void; onClose: () => void }) {
  const { t, i18n } = useTranslation();
  if (!run) return null;
  ...
  return <EntitySheet open onOpenChange={(next) => { if (!next) onClose(); }} eyebrow={t("validation.detailEyebrow")} title={t("validation.validationRunNamed", { id: shortId(run.id) })} description={`${guardrail?.name ?? run.guardrail_id} · ${new Date(run.created_at).toLocaleString(i18n.language)}`} width="xl" footer={<><Button variant="outline" onClick={onClose}>{t("common.close")}</Button><Button disabled={running} onClick={() => onRunAgain(run.guardrail_id)}>{running ? <LoaderCircle className="animate-spin" /> : <Play />}{t("validation.runAgain")}</Button></>}>
    <div className="space-y-5"><div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-muted/20 p-4"><div><p className="text-xs text-muted-foreground">{t("validation.targetColumn")}</p><p className="mt-1 text-sm font-medium">{targetLabel(run, t)}</p></div><StateBadge state={run.status} /></div>
      ...
      <section className="overflow-hidden rounded-xl border"><header className="border-b bg-muted/25 px-4 py-3"><h3 className="text-sm font-semibold">{t("validation.caseResults")}</h3><p className="mt-1 text-xs text-muted-foreground">{t("validation.failuresFirst")}</p></header><div className="divide-y">{results.map((result) => <TestCaseResultRow key={result.case_id} result={result} />)}</div></section>
    </div>
  </EntitySheet>;
}
...
export function TestCaseResultRow({ result }: { result: TestCaseResult }) {
  const { t } = useTranslation();
  return <details open={!result.passed} className="group"><summary className="grid min-h-16 cursor-pointer list-none gap-3 px-4 py-3 focus-visible:outline-2 focus-visible:outline-ring sm:grid-cols-[minmax(0,1fr)_110px_90px_20px] sm:items-center [&::-webkit-details-marker]:hidden"><div><div className="flex flex-wrap items-center gap-2"><strong className="text-sm font-medium">{result.name}</strong><Badge variant="outline" className="font-normal">{testCaseTypeLabel(result.case_type, t)}</Badge></div><p className="mt-1 text-xs text-muted-foreground">{humanize(result.policy_id)} · {humanize(result.phase)}</p></div><StateBadge state={result.actual_decision} /><span className="font-mono text-xs text-muted-foreground">{result.latency_ms} ms</span><ChevronDown className="size-4 text-muted-foreground transition-transform group-open:rotate-180" /></summary><div className="border-t bg-muted/15 p-4">{result.source_policy_id ? <section className="mb-3 rounded-lg border bg-card p-3"><h4 className="text-xs font-medium text-muted-foreground">{t("validation.acceptanceProvenance")}</h4><dl className="mt-3 grid gap-3 text-xs sm:grid-cols-2"><div><dt className="text-muted-foreground">{t("validation.sourcePolicy")}</dt><dd className="mt-1 break-all font-mono">{result.source_policy_id}@{result.source_policy_version}</dd></div><div><dt className="text-muted-foreground">{t("validation.sourceTestCase")}</dt><dd className="mt-1 break-all font-mono">{result.source_case_id}</dd></div><div><dt className="text-muted-foreground">{t("validation.coveredRules")}</dt><dd className="mt-1 flex flex-wrap gap-1">{result.covered_rule_ids.map((id) => <code key={id} className="rounded bg-muted px-1.5 py-0.5 text-xs">{id}</code>)}</dd></div><div><dt className="text-muted-foreground">{t("validation.matchedRules")}</dt><dd className="mt-1 flex flex-wrap gap-1">{result.matched_rule_ids.length ? result.matched_rule_ids.map((id) => <code key={id} className="rounded bg-muted px-1.5 py-0.5 text-xs">{id}</code>) : <span>{t("validation.noRulesMatched")}</span>}</dd></div></dl></section> : null}<div className="grid gap-3 sm:grid-cols-2"><div className="rounded-lg border bg-card p-3"><p className="text-xs text-muted-foreground">{t("guardrails.expectedDecision")}</p><div className="mt-2"><StateBadge state={result.expected_decision} /></div></div><div className="rounded-lg border bg-card p-3"><p className="text-xs text-muted-foreground">{t("guardrails.actualDecision")}</p><div className="mt-2"><StateBadge state={result.actual_decision} /></div></div></div><p className="mt-3 text-sm leading-6">{result.reason}</p>{result.trace.length ? <div className="mt-3 space-y-2">{result.trace.map((step) => <div key={step.id} className="flex gap-3 rounded-lg border bg-card p-3 text-xs"><strong>{step.name}</strong><span className="min-w-0 flex-1 text-muted-foreground">{step.detail}</span><span className="font-mono text-muted-foreground">{step.duration_ms} ms</span></div>)}</div> : null}</div></details>;
}
