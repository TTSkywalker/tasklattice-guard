...
import { ChevronDown, ChevronRight, Info, LoaderCircle, Play, Plus, Search, Trash2 } from "lucide-react";
...
function ValidationDetailSheet({ run, guardrail, running, onRunAgain, onClose }: { run: ValidationRun | null; guardrail?: Guardrail; running: boolean; onRunAgain: (guardrailId: string) => void; onClose: () => void }) {
  ...
  return <EntitySheet open density="compact" onOpenChange={(next) => { if (!next) onClose(); }} eyebrow={t("validation.detailEyebrow")} title={t("validation.validationRunNamed", { id: shortId(run.id) })} description={`${guardrail?.name ?? run.guardrail_id} · ${new Date(run.created_at).toLocaleString(i18n.language)}`} width="xl" footer={<><Button variant="outline" onClick={onClose}>{t("common.close")}</Button><Button disabled={running} onClick={() => onRunAgain(run.guardrail_id)}>{running ? <LoaderCircle className="animate-spin" /> : <Play />}{t("validation.runAgain")}</Button></>}>
    <div className="space-y-3">
      ...
      <ValidationCaseResults key={run.id} results={results} defaultFilter={run.status === "failed" ? "failed" : "all"} />
    </div>
  </EntitySheet>;
}
...
export function ValidationCaseResults({ results, defaultFilter = "all" }: { results: TestCaseResult[]; defaultFilter?: CaseResultFilter }) {
  const { t } = useTranslation();
