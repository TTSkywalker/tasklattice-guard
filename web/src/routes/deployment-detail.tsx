  ...
  return (
    <section className="py-6 sm:py-8">
      ...
      <div className="mt-3 flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        ...
        {!deployment.system_managed ? <Button variant="outline" onClick={() => { setSection("traffic"); setEditScopeOpen(true); }}><Pencil />{t("deploymentDetail.editSelector")}</Button> : null}
      </div>
      ...
      <Tabs value={section} onValueChange={setSection} className="mt-7">
        ...
        <TabsContent value="traffic" className="pt-5">
          <DeploymentTrafficView deployment={deployment} integration={integration} onEdit={() => setEditScopeOpen(true)} />
        </TabsContent>
      </Tabs>
...
function DeploymentRuntimeView({ metrics, metricsLoading, metricsError, traces, tracesLoading, tracesError, policies, onInspect, onOpenSecurity }: { metrics?: Metrics; metricsLoading: boolean; metricsError: unknown; traces: DeploymentRuntimeTrace[]; tracesLoading: boolean; tracesError: unknown; policies: Policy[]; onInspect: (trace: DeploymentRuntimeTrace) => void; onOpenSecurity: () => void }) {
  ...
  return (
    <div className="space-y-4">
      <div><h2 className="text-base font-semibold">{t("deploymentDetail.runtimeTitle")}</h2><p className="mt-1 text-sm text-muted-foreground">{t("deploymentDetail.runtimeDescription")}</p></div>
      ...
      <Card className="shadow-none">
        <CardHeader className="border-b"><CardTitle>{t("deploymentDetail.eventLog")}</CardTitle><CardDescription>{t("deploymentDetail.eventLogDescription")}</CardDescription></CardHeader>
        <CardContent className="p-0">
          {tracesLoading ? <Skeleton className="m-4 h-44 rounded-lg" /> : tracesError ? <div className="p-4"><ErrorNotice error={tracesError} /></div> : traces.length ? <div className="overflow-x-auto"><Table><TableHeader><TableRow className="hover:bg-transparent"><TableHead className="min-w-44 px-4">{t("deploymentDetail.time")}</TableHead><TableHead>{t("deploymentDetail.phase")}</TableHead><TableHead>{t("deploymentDetail.decision")}</TableHead><TableHead>{t("deploymentDetail.finding")}</TableHead><TableHead>{t("deploymentDetail.latency")}</TableHead><TableHead className="w-20"><span className="sr-only">{t("deploymentDetail.inspect")}</span></TableHead></TableRow></TableHeader><TableBody>{traces.map((trace) => <TableRow key={trace.id}><TableCell className="px-4 font-mono text-xs text-muted-foreground">{new Date(trace.created_at).toLocaleString(i18n.language)}</TableCell><TableCell className="text-xs uppercase">{trace.phase}</TableCell><TableCell><StateBadge state={trace.outcome} /></TableCell><TableCell>{trace.severity ? <div className="flex items-center gap-2"><SeverityBadge severity={trace.severity} /><span className="max-w-56 truncate text-xs">{findingTitle(trace.findings[0], policies)}</span></div> : <span className="text-xs text-muted-foreground">{t("deploymentDetail.noFinding")}</span>}</TableCell><TableCell className="font-mono text-xs">{trace.latency_ms} ms</TableCell><TableCell><Button type="button" size="icon" variant="ghost" className="size-11" aria-label={t("deploymentDetail.inspectTrace", { id: trace.id })} onClick={() => onInspect(trace)}><ArrowUpRight className="size-4" /></Button></TableCell></TableRow>)}</TableBody></Table></div> : <div className="p-4"><EmptyState title={t("deploymentDetail.noEventsTitle")} description={t("deploymentDetail.noEventsDescription")} /></div>}
        </CardContent>
      </Card>
    </div>
  );
...
function DeploymentSecurityView({ traces, loading, error, policies, onInspect }: { traces: DeploymentRuntimeTrace[]; loading: boolean; error: unknown; policies: Policy[]; onInspect: (trace: DeploymentRuntimeTrace) => void }) {
  ...
  return <div className="space-y-4">
    ...
    <Card className="shadow-none">
      ...
      <CardContent className="p-0">
        {loading ? <Skeleton className="m-4 h-44 rounded-lg" /> : error ? <div className="p-4"><ErrorNotice error={error} /></div> : visibleFindings.length ? <div className="divide-y">{visibleFindings.map(({ trace, finding }) => <button key={`${trace.id}:${finding.id}`} type="button" className="flex min-h-20 w-full items-start gap-4 px-4 py-4 text-left outline-none hover:bg-muted/35 focus-visible:bg-muted/35 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring/30" onClick={() => onInspect(trace)}><SeverityBadge severity={finding.severity} /><span className="min-w-0 flex-1"><strong className="block truncate text-sm">{findingTitle(finding, policies)}</strong><span className="mt-1 block truncate text-xs text-muted-foreground">{finding.detail}</span><span className="mt-2 block font-mono text-[11px] text-muted-foreground">{finding.policy_id ?? "—"}{finding.rule_id ? ` · ${finding.rule_id}` : ""}</span></span><span className="hidden shrink-0 text-right sm:block"><time className="block font-mono text-xs text-muted-foreground">{new Date(trace.created_at).toLocaleString(i18n.language)}</time><span className="mt-1 block text-xs text-muted-foreground">{Math.round(finding.confidence * 100)}% · {trace.phase} · {trace.latency_ms} ms</span></span><ArrowUpRight className="mt-0.5 size-4 shrink-0 text-muted-foreground" /></button>)}</div> : <div className="flex min-h-48 flex-col items-center justify-center px-6 py-10 text-center"><span className="grid size-10 place-items-center rounded-full bg-muted text-muted-foreground"><ShieldCheck className="size-5" /></span><p className="mt-3 text-sm font-medium">{t(findings.length ? "deploymentDetail.noMatchingFindings" : "deploymentDetail.noSecurityTitle")}</p><p className="mt-1 max-w-lg text-xs leading-5 text-muted-foreground">{t(findings.length ? "deploymentDetail.noMatchingFindingsDescription" : "deploymentDetail.noSecurityDescription")}</p></div>}
      </CardContent>
    </Card>
  </div>;
}
...
function TraceDetailSheet({ trace, deployment, integration, guardrailName, policies, open, onOpenChange }: { trace: DeploymentRuntimeTrace | null; deployment: Deployment; integration?: Integration; guardrailName?: string; policies: Policy[]; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return <EntitySheet open={open} onOpenChange={onOpenChange} eyebrow={t("deploymentDetail.traceEyebrow")} title={t("deploymentDetail.traceTitle")} description={trace.id} width="xl" footer={<Button variant="outline" onClick={() => onOpenChange(false)}>{t("common.close")}</Button>}>
    <div className="grid gap-5">
      ...
      <section><div className="mb-3"><h3 className="text-sm font-semibold">{t("deploymentDetail.findings")}</h3><p className="mt-1 text-xs text-muted-foreground">{t("deploymentDetail.findingsPrivacy")}</p></div>{trace.findings.length ? <div className="grid gap-3">{trace.findings.map((finding) => <div key={finding.id} className="rounded-lg border p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><SeverityBadge severity={finding.severity} /><strong className="text-sm">{findingTitle(finding, policies)}</strong></div><p className="mt-2 text-xs leading-5 text-muted-foreground">{finding.detail}</p></div><span className="font-mono text-xs text-muted-foreground">{Math.round(finding.confidence * 100)}%</span></div><dl className="mt-4 grid gap-3 sm:grid-cols-3"><InlineFact label={t("deploymentDetail.policy")} value={finding.policy_id ?? "—"} mono /><InlineFact label={t("deploymentDetail.rule")} value={finding.rule_id ?? "—"} mono /><InlineFact label={t("deploymentDetail.action")} value={finding.recommended_action} mono /></dl></div>)}</div> : <EmptyState title={t("deploymentDetail.noFinding")} description={t("deploymentDetail.noFindingDescription")} />}</section>
      <section><div className="mb-3"><h3 className="text-sm font-semibold">{t("deploymentDetail.executionTrace")}</h3><p className="mt-1 text-xs text-muted-foreground">{t("deploymentDetail.executionTraceDescription")}</p></div>{trace.steps.length ? <ol className="relative ml-4 border-l">{trace.steps.map((step) => <li key={step.id} className="relative pb-5 pl-6 last:pb-0"><span className="absolute -left-[13px] top-0 grid size-6 place-items-center rounded-full border bg-background text-primary">{step.kind === "rail" ? <Workflow className="size-3" /> : <FileCode2 className="size-3" />}</span><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-sm font-medium">{step.name}</p><p className="mt-1 font-mono text-xs text-muted-foreground">{step.action_name ?? step.flow_name ?? step.kind}{step.action_version ? `@${step.action_version}` : ""}</p></div><span className="flex items-center gap-2 text-xs text-muted-foreground"><StateBadge state={step.outcome} /><Clock3 className="size-3" />{step.latency_ms} ms</span></div></li>)}</ol> : <EmptyState title={t("deploymentDetail.noStepsTitle")} description={t("deploymentDetail.noStepsDescription")} />}</section>
    </div>
  </EntitySheet>;
}

function RuntimeStat({ label, value, detail, danger = false, onClick, actionLabel }: { label: string; value: string; detail: string; danger?: boolean; onClick?: () => void; actionLabel?: string }) { return <div className={`relative border-b p-4 last:border-b-0 sm:[&:nth-child(odd)]:border-r xl:border-b-0 xl:border-r xl:last:border-r-0 ${onClick ? "group hover:bg-muted/25" : ""}`}><dt className="text-xs text-muted-foreground">{label}</dt><dd className={`mt-1 font-display text-2xl font-semibold ${danger ? "text-red-700" : ""}`}>{value}</dd><p className={`mt-1 text-xs ${onClick ? "text-primary" : "text-muted-foreground"}`}>{detail}</p>{onClick ? <button type="button" className="absolute inset-0 rounded-sm outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring/40" aria-label={actionLabel} onClick={onClick}><ArrowUpRight className="absolute right-3 top-3 size-4 text-muted-foreground transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-foreground" /></button> : null}</div>; }
